...
def plan_build(demo: DemoDef):
    ...
    print("  2. Recommend capture method before capture begins:")
    print("     → HTML Capturer for public pages or acceptable Capturer-profile sign-in.")
    print("     → Native browser/window or monitor capture for already-signed-in browser sessions.")
    print("     → Manual Regale recording if native capture returns zero frames and re-login is not acceptable.")
    print("  3. Ask user to open browser tabs/windows and sign in before capture begins.")
    print("  4. regale_studio_uat-list_capture_targets()")
    print("     → Present monitors, explicit browser/window targets if listed, and Active Window title/bounds.")
    print("  5. Prefer explicit browser/window target; otherwise use browser monitor. Avoid Active Window when Copilot is active.")
    print("  6. Ask user to choose target, then regale_studio_uat-set_capture_target(target=...).")
    print("  7. For monitor targets, use a delayed switch workflow before regale_studio_uat-start_capture().")
    print("  8. For each scene, prompt user to prepare the screen and call regale_studio_uat-start_capture().")
    print("  9. If capture returns zero frames or wrong surface, re-list targets and retry window/monitor capture.")
    print("  10. Add narration/notes/hotspots with available Regale page/object tools.\n")
