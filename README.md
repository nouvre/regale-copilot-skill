...
- install-regale-demo-all-tools.cmd - same, but registers all ~138 Regale tools; recovery path for a build that hit an unregistered tool
...
1) Install the Copilot agent, skill, and MCP config. In PowerShell:
   ...
   The installer auto-detects regale-mcp-bridge.exe, preflights what it needs, verifies
   what it wrote, and offers to restart Copilot. Nothing else is required to use this —
   no git clone, no Python. See WINDOWS_COPILOT_APP.md for the non-developer setup and
   daily-use flow.
...
3) (Optional) Parse a .docx (two-column "What to say" / "What to show"). This is the only
